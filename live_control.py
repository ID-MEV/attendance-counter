...
class LiveVideoThread(QThread):
    ...
    def run(self):
        logger.info(f"스레드 시작: {self.rtsp_url}")
        while self._run_flag:
            if not self.is_stream_connected:
                self._cap = cv2.VideoCapture(self.rtsp_url)
                if self._cap.isOpened():
                    self.is_stream_connected = True
                    self.stream_status_signal.emit(True, f"연결 성공: {self.rtsp_url}")
                else:
                    self.is_stream_connected = False
                    self.stream_status_signal.emit(False, f"연결 실패: {self.rtsp_url}")
            ...
            if ret and frame is not None:
                ...
                self.is_stream_connected = False
                self.stream_status_signal.emit(False, "스트림 끊김.")
                break
...
class LiveControlGUI(QMainWindow):
    def __init__(self):
        ...
        self.rtsp_url = self.camera_urls[3]
    ...
    def init_ui(self):
        ...
        self.current_url_label = QLabel(f"현재 스트림: {self.rtsp_url}")
        self.current_url_label.setStyleSheet("color: #f1c40f; font-size: 14px; font-weight: bold; margin-bottom: 5px;")
    ...
    def start_stream(self):
        self.stop_stream()
        self.add_log(f"-> 스트림 연결 시도: {self.rtsp_url}")
        self.current_url_label.setText(f"현재 스트림: {self.rtsp_url}")
    ...
    def stop_stream(self):
        if self.video_thread and self.video_thread.isRunning():
            ...
            self.video_thread = None
            self.add_log("스트림 연결이 중지되었습니다.")

    def update_stream_status(self, ok, msg):
        self.add_log(msg)
        if not ok: 
    ...
    def update_image(self, img):
        p = QPixmap.fromImage(img).scaled(self.video_label.width(), self.video_label.height(), Qt.AspectRatioMode.KeepAspectRatio)
        self.video_label.setPixmap(p)

    def update_raw_frame(self, frame): self.latest_frame = frame
    ...
    def keyPressEvent(self, event):
        ...
        if self.pending_camera_no is not None:
            if key == Qt.Key.Key_Space:
                self.rtsp_url = self.camera_urls[self.pending_camera_no]
                self.add_log(f"확정: {self.pending_camera_no}번 카메라로 주소 변경")
                self.start_stream()
                self.pending_camera_no = None
                return
            elif key in [Qt.Key.Key_1, Qt.Key.Key_2, Qt.Key.Key_3]:
                pass # 아래 트리거에서 새로운 번호 처리
            else:
                self.add_log("전환 취소됨.")
                self.pending_camera_no = None
                # 취소 후 다른 키 작동을 위해 return 하지 않음

        # 2. 카메라 전환 트리거 (상단 숫자키 1,2,3)
        if key == Qt.Key.Key_1: self.pending_camera_no = 1
        elif key == Qt.Key.Key_2: self.pending_camera_no = 2
        elif key == Qt.Key.Key_3: self.pending_camera_no = 3
        
        if self.pending_camera_no is not None and key in [Qt.Key.Key_1, Qt.Key.Key_2, Qt.Key.Key_3]:
            self.add_log(f"{self.pending_camera_no}번 카메라로 전환할까요? (Space:확정 / Other:취소)")
            self.key_status_label.setText(f"{self.pending_camera_no}번 대기 중")
            return
        ...
        if key in [Qt.Key.Key_Return, Qt.Key.Key_Enter]: 
            ...
            self.preset_input.clear(); self.pending_camera_no = None; self.add_log("입력 초기화")
